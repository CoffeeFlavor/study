...
import com.imooc.mvcdemo.model.FileRequest;
import com.imooc.mvcdemo.model.HandleForm;
...
public class CourseController {
    ...
    public String doUploadFile(@RequestParam("file") MultipartFile file) throws IOException {
        ...
        RuntimeSchema<FileRequest> schema=RuntimeSchema.createFrom(FileRequest.class);

        FileRequest fileRequest=new FileRequest();
        fileRequest.setFile(file);
        fileRequest.setName("你好");
        byte[] bytes= ProtobufIOUtil.toByteArray(fileRequest,schema, LinkedBuffer.allocate(LinkedBuffer.DEFAULT_BUFFER_SIZE));

        FileRequest fileNew=schema.newMessage();
        ProtobufIOUtil.mergeFrom(bytes,fileNew,schema);

        FileUtils.copyInputStreamToFile(fileNew.getFile().getInputStream(), new File("d:\\temp\\imooc\\","good"+ System.currentTimeMillis() + file.getOriginalFilename()));
    ...
    @RequestMapping(value = "/upload-to-song", method = RequestMethod.GET)
    public String showDoForm() {
        return "fileUpload";
    }
